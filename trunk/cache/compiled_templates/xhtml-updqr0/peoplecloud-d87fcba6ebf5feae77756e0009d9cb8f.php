...
$i_output = "";
if ( !isset($this->send->peopleCloud))
{
    
    throw new ezcTemplateRuntimeException( sprintf("The external (use) variable '%s' is not set in template: %s and called from %s", 'peopleCloud',  $this->template->stream,  ( sizeof($this->template->streamStack) >= 2 ? $this->template->streamStack[sizeof($this->template->streamStack) - 2] : 'the application code') ) );
}
...
$t_tags = $this->send->peopleCloud["tags"];
$t_highest = $this->send->peopleCloud["highest"];
